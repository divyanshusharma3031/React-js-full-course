...
function Order() {
  ...
  useEffect(()=>{
    ...
    // fetch.state==="idle" check karna jaruri hai warna infinite loop mai chala jaayga. 
  },[fetcher]);
  ...
  return (
    <div className="space-y-8 px-4 py-6">
      ...
      <ul className="divide-y divide-stone-200 boreder-b border-t">
        {cart.map((item)=>{
          return <OrderItem item={item} ingredients={fetcher.data?.find(ele=>item.pizzaId===ele.id)?.ingredients??[]} key={item.pizzaId} isLoadingIngredients={fetcher.state==="loading"}/>
        })}
      </ul>
